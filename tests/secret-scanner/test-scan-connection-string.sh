#!/usr/bin/env bash
# Test: secret-scanner detects database connection strings with credentials
set -uo pipefail
...
echo 'DATABASE_URL = "postgresql://admin:supersecret@db.example.com:5432/mydb"' > "$TMPFILE"
...
echo "$OUTPUT" | grep -q "\[Reaper\]" || { echo "FAIL: postgres connection string not detected"; exit 1; }
exit 0

#!/usr/bin/env bash
# Test: vuln-detector detects XSS via innerHTML
set -uo pipefail
...
TMPFILE=$(mktemp /tmp/reaper-scan-XXXXXX.js)
cat > "$TMPFILE" << 'XSS'
...
echo "$OUTPUT" | grep -q "CWE-79\|innerHTML\|XSS" || { echo "FAIL: XSS via innerHTML not detected"; exit 1; }
exit 0

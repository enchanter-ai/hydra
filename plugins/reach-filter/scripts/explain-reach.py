...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help='Finding ID as emitted by reach-filter (positional). '
             'Either positional or --finding-id must be provided.',
    )
    parser.add_argument(
        "--finding-id",
        dest="finding_id_flag",
        default=None,
        help='Finding ID (flag form, alternative to positional).',
    )
    parser.add_argument(
        "--findings",
        "--out",
        ...
        help="Path to reach-filtered.jsonl (aliases: --findings, --out)",
    )
    args = parser.parse_args(argv)

    finding_id = args.finding_id or args.finding_id_flag
    if not finding_id:
        parser.error("a finding-id is required (positional or --finding-id)")

    row = _find_row(args.out, finding_id)
    if row is None:
        print(
            f"explain-reach: no finding with id {finding_id!r} in "
            f"{args.out}",
            file=sys.stderr,
        )
        return 1

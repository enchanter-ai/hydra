...
def _finding_id(row: dict[str, Any]) -> str:
    ...
    Prefers an explicit `id` field when present (synthetic test fixtures use
    short IDs like ``V-001``); falls back to a deterministic composite of
    ``file:line:vuln_id`` for live audit rows that lack a pre-assigned id.
    """
    explicit = row.get("id")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        / "state"
        / "audit.jsonl",
        help="Path to vuln-detector audit.jsonl (alias: --findings)",
    )
    parser.add_argument(
        "--graph",
        type=Path,
    ...
    if graph is None:
        print(
            f"reach-filter: {raw} raw | {reachable} reachable | "
            f"0 filtered (graph absent)",
            file=sys.stderr,
        )
    else:
        print(
            f"reach-filter: {raw} raw | {reachable} reachable | "
            f"{raw - reachable} filtered",
            file=sys.stderr,
        )
    return 0

...
# Best-effort sibling integration: hydra-audit-trail's central log.
# Only written if the sibling plugin's state dir already exists; never created.
SIBLING_AUDIT_PATH = PLUGIN_ROOT.parent / "audit-trail" / "state" / "audit.jsonl"
...
def emit_block_audit(tool: str, host: str, allowlist: list[str]) -> None:
    ...
    try:
        append_jsonl_locked(AUDIT_PATH, line)
    except OSError:
        # Audit write failure must not change the block decision.
        pass
    # Best-effort sibling integration: append to hydra-audit-trail/state/audit.jsonl
    # ONLY if the sibling state dir already exists. Do not create it. Any failure
    # is silent — central audit is a nice-to-have, not load-bearing.
    try:
        if SIBLING_AUDIT_PATH.parent.is_dir():
            append_jsonl_locked(SIBLING_AUDIT_PATH, line)
    except OSError:
        pass

...
- `--fail-on-deny` — exit non-zero if any DENY hits found. **PR/release context ONLY — never wire this into a hook** (advisory-only contract per `../vis/packages/core/conduct/hooks.md`).

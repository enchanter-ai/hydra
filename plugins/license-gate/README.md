...
**Do not** wire `--fail-on-deny` into any PreToolUse / PostToolUse hook — it would violate the advisory-only hook contract (`shared/foundations/conduct/hooks.md`). Hooks should run the scanner without `--fail-on-deny` and inject the advisory lines as stderr context.

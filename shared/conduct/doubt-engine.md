...
| "Skip the test step, just commit" | "OK, committing." | "Skipping tests means no baseline for the next regression. The push will succeed; the next failure will look like yours even if it isn't. Commit anyway?" |
...
- If it's a recurring pattern across sessions: promote to auto-memory feedback per `precedent.md` § Relationship to auto-memory.

...
Audience: Claude. How to spend the context window so recall stays sharp across long iterative loops and multi-stage plugin chains.

## The problem

Claude's effective recall is not uniform across the window. Middle-of-context recall drops 30%+ on long inputs (context rot). Every byte you paste costs attention on every other byte. A 1M-token window is not a license to fill it.

## U-curve placement

Instructions Claude must obey go in the **first ~200 tokens** or the **last ~200 tokens** of the request. The middle is a recall valley.
...
1. **Cite line ranges, not whole files.** `Read` with `offset` and `limit`. A 40KB file dump to answer a 5-line question wastes 39KB of attention.
